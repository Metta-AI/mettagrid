...
class IdMap:
    ...
    def _compute_features(self) -> list[ObservationFeatureSpec]:
        ...
        # Named game value observation features (multi-token encoding like inventory)
        for prefix in self._config.obs.global_obs.obs:
        ...
        if self._config.obs.global_obs.last_action_move:
            add_feature("last_action_move", 1.0)

        if self._config.obs.territory:
            add_feature("territory:here", 3.0)
            for direction in ("north", "south", "east", "west"):
                add_feature(f"territory:{direction}", 9.0)
