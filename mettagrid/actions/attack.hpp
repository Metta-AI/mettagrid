...
protected:
    ...
    bool _handle_target(unsigned int actor_id, Agent* actor, CppGridLocation target_loc)
    {
        ...
        if (agent_target) {
            actor->stats.incr(_stats.target[agent_target->objectTypeId]);
            actor->stats.incr(_stats.target[agent_target->objectTypeId], actor->group_name);
            actor->stats.incr(_stats.target[agent_target->objectTypeId], actor->group_name, agent_target->group_name);
            ...
            if (agent_target->inventory[InventoryItem::armor] > 0) {
                ...
                if (!was_frozen) {
                    ...
                    for (int item = 0; item < InventoryItem::InventoryCount; item++) {
                        actor->stats.add(InventoryItemNames[item], "stolen", actor->group_name,
                                         agent_target->inventory[item]);
                        actor->update_inventory(static_cast<InventoryItem>(item), agent_target->inventory[item]);
                        agent_target->update_inventory(static_cast<InventoryItem>(item),
                                                       -agent_target->inventory[item]);
                    }
                }
        ...
        if (object_target) {
            actor->stats.incr(_stats.target[object_target->objectTypeId]);
            actor->stats.incr(_stats.target[object_target->objectTypeId], actor->group_name);
            object_target->hp -= 1;
            actor->stats.incr("damage", ObjectTypeNames[object_target->objectTypeId]);
            if (object_target->hp <= 0) {
                _grid->remove_object(object_target);
                actor->stats.incr("destroyed", ObjectTypeNames[object_target->objectTypeId]);
            }
            return true;
        }
