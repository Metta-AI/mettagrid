...
class GiantMazeWorld(Room):
    ...
    def _build(self):
        # sample widths
        self.cw = int(self._rng.integers(self.corridor_range[0],
                                         self.corridor_range[1] + 1))
        self.wt = int(self._rng.integers(self.wall_range[0],
                                         self.wall_range[1] + 1))
    ...
    def _generate_maze(self) -> np.ndarray:
        ...
        while stack:
            ...
            opts = [
                (dr, dc)
                for dr, dc in dirs
                if 0 < r + dr * 2 < small_h - 1
                and 0 < c + dc * 2 < small_w - 1
                and not coarse[r + dr * 2, c + dc * 2]
            ]
            if opts:
