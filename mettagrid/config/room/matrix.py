...
class Matrix(Room):
    ...
    (excluding blocks). blocks (of thickness `border_width`) separate the rooms,
    but each block has a centrally placed gap of size `door_gap` so that the agent
    ...
    def __init__(self, rows: int, cols: int, room_width: int, room_height: int,
                 door_gap: int, border_width: int = 1, seed: int = None):
        super().__init__(border_width=border_width, border_object="block")
        self.rows = rows
        self.cols = cols
    ...
    def _build(self) -> np.ndarray:
        ...
        # Draw internal horizontal blocks with door gaps.
        for r in range(1, self.rows):
            block_y = r * (self.room_height + self.border_width)
            for c in range(self.cols):
                ...
                # Fill the block cells (within the room's horizontal extent) except for the gap.
                for x in range(room_start_x, room_start_x + self.room_width):
                    if not (gap_start <= x < gap_end):
                        grid[block_y, x] = "block"

        # Draw internal vertical blocks with door gaps.
        for c in range(1, self.cols):
            block_x = c * (self.room_width + self.border_width)
