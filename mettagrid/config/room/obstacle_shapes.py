...
class ObstacleShapes(Room):
    ...
    def _build(self):
        ...
        # Replace wall objects with composite obstacle shapes
    ...
    def _place_obstacles(self, grid, obstacle_count: int):
        """
        Places composite obstacle objects in the grid. Each obstacle is a cross shape defined by a 3x3 pattern,
        and is placed with a one-cell clearance around it to ensure traversability.
        """
        # Define the obstacle cross pattern
        pattern = np.array([
            ["empty", "wall", "empty"],
            ["wall", "wall", "wall"],
            ["empty", "wall", "empty"]
        ...
        clearance = 1
        effective_h = p_h + 2 * clearance  # Total height required (5 cells)
        effective_w = p_w + 2 * clearance  # Total width required (5 cells)
        height, width = grid.shape
        attempts = 0
        placed = 0
        max_attempts = obstacle_count * 100 if obstacle_count > 0 else 0
        ...
        while placed < obstacle_count and attempts < max_attempts:
            ...
            if np.all(region == "empty"):
                # Determine where to place the pattern within the effective region
                start_r = r + clearance
                start_c = c + clearance
