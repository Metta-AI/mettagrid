...
public:
    ...
    Agent(cpp_GridCoord r, cpp_GridCoord c, std::string group_name, unsigned char group_id, ObjectConfig cfg,
          // Configuration -- rewards that the agent will get for certain
          // actions or inventory changes.
          std::map<std::string, float> rewards)
    {
        CppGridObject::cpp_init(ObjectType::AgentT, CppGridLocation(r, c, GridLayer::Agent_Layer));
