...
def ascii_to_patterns_with_counts(source: str, n: int, periodic: bool = False, symmetry: Symmetry = "none") -> list[tuple['Pattern', int]]:
...
def ascii_to_weights_of_all_patterns(source: str, n: int, periodic: bool = False, symmetry: Symmetry = "none") -> npt.NDArray[np.float_]:
...
class Pattern:
    ...
    def __str__(self) -> str:
        return "Pattern:\n" + "\n".join("".join("#" if self.data[y, x] else " " for x in range(self.size())) for y in range(self.size()))
